...
bool handle_drive_around_request(ball_chaser::DriveToTarget::Request& req,
...
//    std::vector<float> joints_angles = clamp_at_boundaries(req.joint_1, req.joint_2);
    ...
    if (ros::ok()) {
        ...
        motor_command.linear.x = linear_x; //0.15;
        motor_command.angular.z = angular_z; //0.05;
        // Publish angles to drive the robot
        motor_command_publisher.publish(motor_command);

	// Return a response message
	res.msg_feedback = "Linear velocity: " + std::to_string(linear_x) + " , angular velocity: " + std::to_string(angular_z);
...
int main(int argc, char** argv)
{
    ...
    ros::ServiceServer service=n.advertiseService("/ball_chaser/command_robot",handle_drive_around_request);
